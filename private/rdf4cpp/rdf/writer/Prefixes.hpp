...
static bool write_fixed_type_iri(datatypes::registry::LiteralType t, void *const buffer, writer::Cursor *const cursor, writer::FlushFunc const flush) noexcept {
    assert(t.is_fixed());

    if constexpr (short_form) {
        auto const &p = detail::type_iri_buffer[t.to_underlying()];
        if (!p.empty()) {
            return write_str(p, buffer, cursor, flush);
        }
    }

    RDF4CPP_DETAIL_TRY_WRITE_STR("<");
    RDF4CPP_DETAIL_TRY_WRITE_STR(datatypes::registry::DatatypeRegistry::get_entry(t)->datatype_iri);
    return write_str(">", buffer, cursor, flush);
}
