...
TEST_CASE("Datatype GDay") {
    ...
    constexpr auto correct_type_iri_cstr = "http://www.w3.org/2001/XMLSchema#gDay";

    CHECK(std::string(datatypes::xsd::GDay::identifier) == correct_type_iri_cstr);

    auto type_iri = IRI(datatypes::xsd::GDay::identifier);
    CHECK(type_iri.identifier() == correct_type_iri_cstr);

    using type = datatypes::xsd::GDay::cpp_type;
    ...
    auto lit1 = Literal::make<datatypes::xsd::GDay>(value);
    CHECK(lit1.value<datatypes::xsd::GDay>() == value);
    ...
    auto lit2 = Literal::make<datatypes::xsd::GDay>(value);
    CHECK(lit2.value<datatypes::xsd::GDay>() == value);
    ...
    auto lit3 = Literal::make<datatypes::xsd::GDay>(value);
    CHECK(lit3.value<datatypes::xsd::GDay>() == value);
    ...
    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("---22\n", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("---22\t", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("---22\r", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("---60", type_iri), "XSD Parsing Error", std::runtime_error);
}