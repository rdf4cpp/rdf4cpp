...
TEST_CASE("Datatype GMonth") {
    ...
    constexpr auto correct_type_iri_cstr = "http://www.w3.org/2001/XMLSchema#gMonth";

    CHECK(std::string(datatypes::xsd::GMonth::identifier) == correct_type_iri_cstr);

    auto type_iri = IRI(datatypes::xsd::GMonth::identifier);
    CHECK(type_iri.identifier() == correct_type_iri_cstr);

    using type = datatypes::xsd::GMonth::cpp_type;
    ...
    auto lit1 = Literal::make<datatypes::xsd::GMonth>(value);
    CHECK(lit1.value<datatypes::xsd::GMonth>() == value);
    ...
    auto lit2 = Literal::make<datatypes::xsd::GMonth>(value);
    CHECK(lit2.value<datatypes::xsd::GMonth>() == value);
    ...
    auto lit3 = Literal::make<datatypes::xsd::GMonth>(value);
    CHECK(lit3.value<datatypes::xsd::GMonth>() == value);
    ...
    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("--05\n", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("--05\t", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("--05\r", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("--13", type_iri), "XSD Parsing Error", std::runtime_error);
}