...
template<>
struct std::formatter<rdf4cpp::BlankNode> : std::formatter<string_view> {
    auto format(rdf4cpp::BlankNode n, format_context &ctx) const -> decltype(std::formatter<std::string_view>::format(std::string_view{}, ctx));
};
