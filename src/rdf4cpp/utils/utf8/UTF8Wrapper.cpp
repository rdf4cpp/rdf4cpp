...
bool rdf4cpp::utils::utf8::UTF8Wrapper::is_valid_utf8_char(std::string &utf8_char) {
    ...
    else if (init_char<=0xDF && init_char >= 0xC2 && utf8_char.length()==2){ // 2byte utf 8 char
        return ((u_char)utf8_char[1])&0x80;
    }
    else if (init_char<=0xEF && init_char>=0xE0 && utf8_char.length()==3){ // 3byte utf 8 char
        return ((u_char)utf8_char[1])&0x80  && ((u_char)utf8_char[2])&0x80;
    }
    else if (init_char<=0xF4 && init_char>=0xF0 && utf8_char.length()==4){ // 4byte utf 8 char
