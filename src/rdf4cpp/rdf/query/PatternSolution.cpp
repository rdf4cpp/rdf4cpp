#include "PatternSolution.hpp"
namespace rdf4cpp::rdf::query {
std::vector<Variable> PatternSolution::extract_variables(const QuadPattern &quad) {
...
Node PatternSolution::operator[](const Variable &variable) const {
...
Node PatternSolution::operator[](size_t pos) const {
    if (pos < current_solution.size())
        return current_solution[pos];
    else
        return {};
}
Variable PatternSolution::variable(size_t pos) const {
    if (pos < variables.size())
...
size_t PatternSolution::bound_count() const {
    return std::count_if(current_solution.begin(), current_solution.end(), [](Node node) { return node != Node{}; });
}
}  // namespace rdf4cpp::rdf::query