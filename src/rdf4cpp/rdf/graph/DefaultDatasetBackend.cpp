#include "DefaultDatasetBackend.h"

namespace rdf4cpp::rdf::graph {
DefaultDatasetBackend::NodeManager &DefaultDatasetBackend::node_storage() const {
...
PatternSolutions DefaultDatasetBackend::match(const QuadPattern &quad_pattern) const {
    return PatternSolutions(quads_, quad_pattern);
}

size_t DefaultDatasetBackend::size(const node::IRIResource &graph_name) const {
    return std::count_if(quads_.begin(), quads_.end(), [&](const Quad &quad) { return (IRIResource) quad.graph() == graph_name; });
}
