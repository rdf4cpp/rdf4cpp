...
using Quad = rdf4cpp::rdf::graph::Quad;
using namespace rdf4cpp::rdf::node;

struct SerdHandle {
    std::map<std::string, std::string> prefixes;
    rdf4cpp::rdf::graph::Dataset *datset;
};
...
IRIResource getURI(const SerdNode *node) {
    auto iri = std::string(std::string_view{(char *) (node->buf), size_t(node->n_bytes)});
    return IRIResource(iri);
}

IRIResource getPrefixedUri(SerdHandle &handle, const SerdNode *node) {
    ...
    return IRIResource(full_string);
}
...
Literal getLiteral(const SerdNode *literal, const SerdNode *type_node, const SerdNode *lang_node) {
    ...
    if (type_node != nullptr)
        return Literal(literal_value,
                       IRIResource(std::string{(char *) (type_node->buf), size_t(type_node->n_bytes)}));
...
SerdStatus on_statement(SerdHandle *handle,
    ...
    if (graph != nullptr)
        ...
        quad.graph() = IRIResource::default_graph();
