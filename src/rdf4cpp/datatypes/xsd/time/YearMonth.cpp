...
bool capabilities::Default<xsd_gYearMonth>::serialize_canonical_string(cpp_type const &value, writer::BufWriterParts writer) noexcept {
    //year,-,month,tz
    std::array<char, 6+1+3 + Timezone::max_canonical_string_chars> buff;
    char *it = std::format_to(buff.data(), "{:%Y-%m}", value.first);
...
std::optional<storage::identifier::LiteralID> capabilities::Inlineable<xsd_gYearMonth>::try_into_inlined(cpp_type const &value) noexcept {
    if (value.second.has_value())
        return std::nullopt;
    return util::pack<storage::identifier::LiteralID>(InliningHelperYearMonth{static_cast<int16_t>(static_cast<int>(value.first.year())),
