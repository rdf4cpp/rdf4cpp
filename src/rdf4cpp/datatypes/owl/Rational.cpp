...
capabilities::Default<owl_rational>::cpp_type capabilities::Default<owl_rational>::from_string(std::string_view s) {
    if (auto pos = s.find_last_of('-'); pos != std::string_view::npos && pos != 0) {
        ...
        throw InvalidNode{"http://www.w3.org/2002/07/owl#rational parsing error: invalid sign position"};
    }

    if (s.find_first_not_of("0123456789/-") != std::string_view::npos) {
        // owl:rational does not allow hex, boost does
        throw InvalidNode{"http://www.w3.org/2002/07/owl#rational parsing error: invalid character in string"};
    }

    try {
        return cpp_type{s};
    } catch (std::runtime_error const &e) {
        throw InvalidNode{std::string{"http://www.w3.org/2002/07/owl#rational parsing error: "} + e.what()};
    }
}
